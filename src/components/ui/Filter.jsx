	...
	return (
		<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-gradient-to-r from-green-100 via-green-50 to-green-100 border border-green-300 rounded-2xl shadow-lg p-5 mb-8">
			...
			<div className="flex flex-col sm:flex-row gap-3 sm:items-center">
				...
				<button
					disabled={sortBy === "number"}
					onClick={() => setOrder(order === "asc" ? "desc" : "asc")}
					className={`
						px-4 py-2 rounded-lg font-semibold shadow-sm transition-all duration-200
						${
							sortBy === "number"
				...
				<button
					...
					className="flex items-center gap-2 px-4 py-2 rounded-lg bg-red-500 hover:bg-red-600 text-white font-semibold shadow-sm transition-all duration-200"
				>
